...
void bubble(long unsigned *vector, int useness, int lenght){
	register int i, j;
	for(i = 1; i < lenght; i++)
...
//implementação feita pela ICMC (Instituto de Ciências Matemática e de Computação)
...
//implementação aleatória que achei por aí
...
void sort_methods(long unsigned *vector, double (*times)[N_FUNCS], int tests, long unsigned lenght){
	...
	copy_vector(shadow, vector, lenght); //copiando sombra
	meta_func(shadow, bubble, times, lenght, 0);
	copy_vector(shadow, vector, lenght); //copiando sombra
	meta_func(shadow, merge, times, lenght, 1);
	copy_vector(shadow, vector, lenght); //copiando sombra
	meta_func(shadown, quick_sort, times, lenght, 2);
